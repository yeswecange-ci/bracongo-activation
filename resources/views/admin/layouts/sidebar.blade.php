<aside class="w-64 bg-gradient-to-b from-gray-900 via-gray-900 to-gray-800 text-white flex-shrink-0 flex flex-col h-screen shadow-2xl" x-data="{ open: true }">
    ...
    <nav class="mt-4 flex-1 overflow-y-auto pb-6 px-3 space-y-1">
        ...
        <!-- LCK Paramètres -->
        <a href="{{ route('admin.lck.settings') }}"
            class="flex items-center px-4 py-3 text-gray-300 hover:bg-gray-800 hover:text-white transition-all rounded-lg group {{ request()->routeIs('admin.lck.settings*') ? 'bg-gradient-to-r from-yellow-700 to-yellow-800 text-white shadow-lg' : '' }}">
            <svg class="w-5 h-5 mr-3 {{ request()->routeIs('admin.lck.settings*') ? 'text-white' : 'text-gray-400 group-hover:text-yellow-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <!-- Rapport global -->
        <a href="{{ route('admin.rapport') }}"
            class="flex items-center px-4 py-3 text-gray-300 hover:bg-gray-800 hover:text-white transition-all rounded-lg group {{ request()->routeIs('admin.rapport*') ? 'bg-gradient-to-r from-emerald-600 to-emerald-700 text-white shadow-lg' : '' }}">
            <svg class="w-5 h-5 mr-3 {{ request()->routeIs('admin.rapport*') ? 'text-white' : 'text-gray-400 group-hover:text-emerald-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z">
                </path>
            </svg>
            <span class="font-medium">Rapport global</span>
        </a>
    </nav>
</aside>
