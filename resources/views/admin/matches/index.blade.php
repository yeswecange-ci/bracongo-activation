...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between bg-white rounded-xl shadow-sm p-6">
        <div class="mb-4 md:mb-0">
            ...
            <p class="text-sm text-gray-500 mt-2">Gérez tous les matchs de la CAN 2025</p>
        </div>
        <a href="{{ route('admin.matches.create') }}" class="inline-flex items-center bg-gradient-to-r from-orange-600 to-orange-700 text-white px-6 py-3 rounded-lg hover:from-orange-700 hover:to-orange-800 transition-all shadow-md hover:shadow-lg transform hover:-translate-y-0.5">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gradient-to-r from-gray-50 to-gray-100">
                    <tr>
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">Match</th>
                        ...
                        <th class="px-6 py-4 text-center text-xs font-bold text-gray-700 uppercase tracking-wider">Pronostics</th>
                        ...
                        <tr class="hover:bg-orange-50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center space-x-3">
                                    <div class="flex-shrink-0">
                                        <div class="w-10 h-10 bg-gradient-to-br from-orange-400 to-orange-600 rounded-lg flex items-center justify-center text-white font-bold">
                                    ...
                                    <div>
                                        <div class="text-sm font-bold text-gray-900">{{ $match->team_a }}</div>
                                        <div class="text-xs text-gray-500">vs</div>
                                        <div class="text-sm font-bold text-gray-900">{{ $match->team_b }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center text-sm">
                                    ...
                                    <div>
                                        <div class="font-medium text-gray-900">{{ $match->match_date->format('d/m/Y') }}</div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                @if($match->status === 'finished' && $match->score_a !== null && $match->score_b !== null)
                                    <div class="inline-flex items-center space-x-2 bg-gray-900 text-white px-4 py-2 rounded-lg font-bold text-lg">
                                        <span>{{ $match->score_a }}</span>
                                        <span class="text-gray-400">-</span>
                                        <span>{{ $match->score_b }}</span>
                                    </div>
                                @else
                                    <span class="text-sm text-gray-400 italic">À venir</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                ...
                                @elseif($match->status === 'live')
                                    <span class="inline-flex items-center px-3 py-1 text-xs font-bold rounded-full bg-green-100 text-green-800 animate-pulse">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end space-x-2">
                                    ...
                                    <a href="{{ route('admin.matches.edit', $match) }}" class="p-2 text-orange-600 hover:bg-orange-50 rounded-lg transition-colors" title="Modifier">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                        </svg>
                                    </a>
                                    <form action="{{ route('admin.matches.destroy', $match) }}" method="POST" class="inline" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce match ?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                ...
                                <p class="text-gray-500 font-medium">Aucun match trouvé</p>
                                <a href="{{ route('admin.matches.create') }}" class="inline-block mt-4 text-orange-600 hover:text-orange-800 font-semibold">
